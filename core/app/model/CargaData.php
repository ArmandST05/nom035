...
class CargaData {
    ...
    public function __construct($empresa_id = null) {
        $this->db = Database::getCon(); // Usamos tu método estático para obtener la conexión
        $this->empresa_id = $empresa_id;  // Guardamos el empresa_id pasado al constructor
    }
    ...
    public function saveToDatabase($data) {
        ...
        // Llamar a la función que hace el INSERT
        return $this->insertIntoDatabase($col1, $id_puesto, $id_departamento, $col4, $col5, $usuario, $clave, $fecha_alta);
    }

    private function insertIntoDatabase($nombre, $id_puesto, $id_departamento, $correo, $telefono, $usuario, $clave, $fecha_alta) {
        // Verificar si empresa_id está configurado antes de hacer el insert
        if ($this->empresa_id === null) {
            return "Error: No se ha definido el ID de la empresa.";
        }
        ...
        $stmt->bind_param("siiisssss", $nombre, $id_puesto, $id_departamento, $correo, $telefono, $usuario, $clave, $fecha_alta, $this->empresa_id);
